...
# Relationship between SPEAR pesticides, estimated toxicity and other environmental factors ---- 
# _______________________________________________________________________________________________
...
source(file.path(path_src, "TU_Trait_TPG_relationships", "07_01_SPEAR_taxa_join_via_order.R"))
...
# Merge other environmental factors
...
# Scatterplot
ggplot(
  result_spear, aes(x = max_log_tu, y = SPEAR_Pestizide)) +
  geom_point() +
  facet_wrap(~ Region) +
  theme_bw() +
  theme(
...
ggsave(
  file.path(path_paper, "Figures", "spear_maxTU.png"),
  width = 12,
  height = 6.5
)
...
for (i in unique(result_spear$Region)) {
  lm_spear[[i]] <- lm(SPEAR_Pestizide ~ max_log_tu + Riffle.FRC + Temp.median + orthoP_4wk.median,
...
# Add R2 and deviance 
spear_gof <- lapply(lm_spear, broom::glance) |> rbindlist(idcol = "Region")
spear_table[spear_gof, `:=`(r.squared = i.r.squared, adj.r.squared = i.adj.r.squared), on = "Region"]

## Type II Sum of squares ----
# Test each main effect after accounting for the other main effects (assuming no interactions)
# Essentially, we could also create 4 models each missing the relevant predictor and then compare them
# in 4 Anovas against the full model
# -> We get the amount of unexplained variance a specific factor helps to explain after accounting for the 
# other factors
# https://blog.msbstats.info/posts/2021-05-25-everything-about-anova/
spear_type2ss <- lapply(lm_spear, function(x) car::Anova(x, type=2, test="F"))
spear_type2ss <- lapply(spear_type2ss, broom::tidy) |> rbindlist(idcol="Region")

# Calc. prop. explained variance -> Performance measure
spear_type2ss[, total_var := sum(sumsq), by = "Region"]
spear_type2ss[, prop_var := (sumsq/total_var)*100]
# spear_type2ss[term=="max_log_tu", ]
# saveRDS(spear_type2ss, file.path(path_cache, "spear_type2ss.rds"))

# Average across regions per expl. variable
spear_type2ss[, mean(prop_var), by = "term"]

# Calc. N of significant marginal effects -> consistency of significance
spear_type2ss[p.value <= 0.05, .N, by="term"]

## Check the direction of the effect (parameter level) ----
spear_table[, dir_estimate := sign(estimate)]

# Positive: only the intercept 
spear_table[p.value <=0.05 & dir_estimate==1, sum(dir_estimate), by="term"]

# Negative: max_log_tu in 5 regions and phosphate in 2 regions
spear_table[p.value <=0.05 & dir_estimate==-1, sum(dir_estimate), by="term"]
...
fwrite(spear_table, file.path(path_paper, "Tables", "SPEAR_regr_results.csv"))
