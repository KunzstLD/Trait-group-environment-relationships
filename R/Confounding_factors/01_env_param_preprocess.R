...
# TODO:
# - Which variables to use for habitat and nutrients?
...
# Plot correlating with consistent CWM traits
pl_habitat <- list()
...
for (region in names(data_cwm_env)) {
    ...
    names(output) <- titles_traits
    regr_interactions[[region]] <- rbindlist(
        output,
        idcol = "cwm_trait"
    )
}